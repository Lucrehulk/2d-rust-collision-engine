...
use std::{ptr, collections::HashSet, sync::atomic::{AtomicBool, Ordering}};
use wide::f32x4;
use rayon;
...
fn manage_collision_circle_circle(dsq: f32, entity: &Entity, collision_entity: &Entity, applied_collisions_ptrs_ptr: *mut [f32; 4], thread: usize, entities_size: usize, stored_collisions: &mut Vec<[usize; 2]>) {
    if (entity.collision_type == collision_entity.collision_type
    && entity.collision_type != 0)
...
fn manage_collision_circle_square(cx: f32, cy: f32, circle: &Entity, square: &Entity, applied_collisions_ptrs_ptr: *mut [f32; 4], thread: usize, entities_size: usize, stored_collisions: &mut Vec<[usize; 2]>) {
    if (circle.collision_type == square.collision_type
    && circle.collision_type != 0)
...
#[inline(always)]
fn update_grid_position(pos: usize, index: usize, spatial_grid: *mut HashSet<usize>, spatial_grid_locks: *mut AtomicBool) {
    unsafe { 
        while (*spatial_grid_locks.add(pos)).compare_exchange_weak(false, true, Ordering::Acquire, Ordering::Relaxed).is_err() {
            std::hint::spin_loop();
        }
...
#[inline(always)]
fn remove_grid_position(pos: usize, index: &usize, spatial_grid: *mut HashSet<usize>, spatial_grid_locks: *mut AtomicBool) {
    unsafe { 
        while (*spatial_grid_locks.add(pos)).compare_exchange_weak(false, true, Ordering::Acquire, Ordering::Relaxed).is_err() {
            std::hint::spin_loop();
        }
...
#[inline(always)]
fn update_entity_body(y_bound: usize, x_bound: usize, y_position_update: usize, x_position_update: usize, y_position_delete: usize, x_position_delete: usize, index: usize, spatial_grid: *mut HashSet<usize>, spatial_grid_locks: *mut AtomicBool) {
...
impl Room {
    ...
    #[inline(always)]
    pub fn create_entity(&mut self, mut x: f32, mut y: f32, mass: f32, friction: f32, velocity_x: f32, velocity_y: f32, terminal_velocity_in_direction: f32, movement_acceleration: f32, radius: f32, body_type: u8, collision_type: u8) -> usize {
        if x - radius < 0.0 { 
            x = radius;
        ...
        for y_pos in grid_pos_y..usize::min(grid_pos_y + grid_body, SPATIAL_GRID_DIMENSION) {
            for x_pos in grid_pos_x..usize::min(grid_pos_x + grid_body, SPATIAL_GRID_DIMENSION) {
                update_grid_position((y_pos << ENCODING_BITS) | x_pos, index, spatial_grid_ptr, spatial_grid_locks_ptr);
            };
        };
        index
    }
    ...
    pub fn resize_entity(&mut self, index: usize, delta_size: f32) -> f32 {
        ...
        for y_pos in entity.grid_pos_y..usize::min(entity.grid_pos_y + entity.grid_body, SPATIAL_GRID_DIMENSION) {
            for x_pos in entity.grid_pos_x..usize::min(entity.grid_pos_x + entity.grid_body, SPATIAL_GRID_DIMENSION) {
                remove_grid_position((y_pos << ENCODING_BITS) | x_pos, &index, spatial_grid_ptr, spatial_grid_locks_ptr);
            }
        }
    ...
    pub fn remove_entity(&mut self, index: usize) {
        ...
        for y_pos in entity.grid_pos_y..usize::min(entity.grid_pos_y + entity.grid_body, SPATIAL_GRID_DIMENSION) {
            for x_pos in entity.grid_pos_x..usize::min(entity.grid_pos_x + entity.grid_body, SPATIAL_GRID_DIMENSION) {
                remove_grid_position((y_pos << ENCODING_BITS) | x_pos, &entity.index, spatial_grid_ptr, spatial_grid_locks_ptr);
            };
        };
    }
    ...
    pub fn create_entity_movement_from_cardinal_direction(&mut self, index: usize, direction: u8) {
        let entity = &mut self.entities[index];
        match direction {
            0 => { entity.acceleration_x = entity.movement_acceleration; entity.acceleration_y = 0.0; }
            1 => { entity.acceleration_x = -entity.movement_acceleration; entity.acceleration_y = 0.0; }
            2 => { entity.acceleration_y = entity.movement_acceleration; entity.acceleration_x = 0.0; }
            3 => { entity.acceleration_y = -entity.movement_acceleration; entity.acceleration_x = 0.0; }
            4 => { entity.acceleration_x = entity.movement_acceleration * 0.70710675; entity.acceleration_y = entity.movement_acceleration * 0.70710675; }
            5 => { entity.acceleration_x = entity.movement_acceleration * 0.70710675; entity.acceleration_y = -entity.movement_acceleration * 0.70710675; }
            6 => { entity.acceleration_x = -entity.movement_acceleration * 0.70710675; entity.acceleration_y = entity.movement_acceleration * 0.70710675; }
            7 => { entity.acceleration_x = -entity.movement_acceleration * 0.70710675; entity.acceleration_y = -entity.movement_acceleration * 0.70710675; }
            _ => { entity.acceleration_x = 0.0; entity.acceleration_y = 0.0; }
        }
    }
    ...
    pub fn update(&mut self) {
                        ...
                        if !entity.replace {
                            ...
                            if spatial_grid_pos_x != entity.grid_pos_x || spatial_grid_pos_y != entity.grid_pos_y {
                                ...
                                if shift_x.0 > entity.grid_body || shift_y.0 > entity.grid_body {
                                    update_entity_body(entity.grid_body, entity.grid_body, spatial_grid_pos_y, spatial_grid_pos_x, entity.grid_pos_y, entity.grid_pos_x, entity.index, spatial_grid, spatial_grid_locks);
                                } else {
                                    if shift_x.0 == 0 {
                                        if shift_y.1 {
                                            update_entity_body(shift_y.0, entity.grid_body, entity.grid_pos_y + entity.grid_body, spatial_grid_pos_x, entity.grid_pos_y, spatial_grid_pos_x, entity.index, spatial_grid, spatial_grid_locks);
                                        } else {
                                            update_entity_body(shift_y.0, entity.grid_body, spatial_grid_pos_y, spatial_grid_pos_x, spatial_grid_pos_y + entity.grid_body, spatial_grid_pos_x, entity.index, spatial_grid, spatial_grid_locks);
                                        };
                                    } else if shift_y.0 == 0 {
                                        if shift_x.1 {
                                            update_entity_body(entity.grid_body, shift_x.0, spatial_grid_pos_y, entity.grid_pos_x + entity.grid_body, spatial_grid_pos_y, entity.grid_pos_x, entity.index, spatial_grid, spatial_grid_locks);
                                        } else {
                                            update_entity_body(entity.grid_body, shift_x.0, spatial_grid_pos_y, spatial_grid_pos_x, spatial_grid_pos_y, spatial_grid_pos_x + entity.grid_body, entity.index, spatial_grid, spatial_grid_locks);
                                        };
                                    } else {
                                        if shift_y.1 {
                                            update_entity_body(shift_y.0, entity.grid_body, spatial_grid_pos_y + entity.grid_body - shift_y.0, spatial_grid_pos_x, entity.grid_pos_y, entity.grid_pos_x, entity.index, spatial_grid, spatial_grid_locks);
                                        } else {
                                            update_entity_body(shift_y.0, entity.grid_body, spatial_grid_pos_y, spatial_grid_pos_x, entity.grid_pos_y + entity.grid_body - shift_y.0, entity.grid_pos_x, entity.index, spatial_grid, spatial_grid_locks);
                                        };
                                        let y_overlap_start = usize::max(spatial_grid_pos_y, entity.grid_pos_y);
                                        if shift_x.1 {
                                            update_entity_body(entity.grid_body - shift_y.0, shift_x.0, y_overlap_start, spatial_grid_pos_x + entity.grid_body - shift_x.0, y_overlap_start, entity.grid_pos_x, entity.index, spatial_grid, spatial_grid_locks);
                                        } else {
                                            update_entity_body(entity.grid_body - shift_y.0, shift_x.0, y_overlap_start, spatial_grid_pos_x, y_overlap_start, entity.grid_pos_x + entity.grid_body - shift_x.0, entity.index, spatial_grid, spatial_grid_locks);
                                        };
                                    };
                                };
        ...
        if self.applied_collisions.capacity() < required_len {
            self.applied_collisions.reserve(required_len - self.applied_collisions.len());
        }
                        ...
                        for entity_index in chunk[0]..chunk[1] {
                            ...
                            for y_pos in entity.grid_pos_y..usize::min(entity.grid_pos_y + entity.grid_body, SPATIAL_GRID_DIMENSION) {
                                for x_pos in entity.grid_pos_x..usize::min(entity.grid_pos_x + entity.grid_body, SPATIAL_GRID_DIMENSION) {
                                    ...
                                    if spatial_grid_position.len() <= 1 { continue };
                                    
                                    for &collision_index in spatial_grid_position.iter() {
                                        if entity_index >= collision_index { continue };
