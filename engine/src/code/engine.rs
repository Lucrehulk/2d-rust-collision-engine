...
pub struct Room {
    ...
    entity_chunks: [[usize; 2]; THREADS],
    collision_chunks: [[usize; 2]; THREADS],
...
#[inline(always)]
fn manage_collision_circle_circle(dsq: f32, entity: &Entity, collision_entity: &Entity, applied_collisions_ptrs_ptr: *mut [f32; 4], thread: usize, entities_size: usize) {
    ...
    let separation = (entity.radius + collision_entity.radius) - distance + 0.00001;
...
#[inline(always)]
fn manage_collision_circle_square(cx: f32, cy: f32, circle: &Entity, square: &Entity, applied_collisions_ptrs_ptr: *mut [f32; 4], thread: usize, entities_size: usize) {
    let dx = circle.x - cx;
    let dy = circle.y - cy;
    ...
    let (nx, ny, separation) = if dist_sq == 0.0 {
        ...
        if min_dist == d_left { (-1.0, 0.0, circle.radius + min_dist + 0.00001) }
        else if min_dist == d_right { (1.0, 0.0, circle.radius + min_dist + 0.00001) }
        else if min_dist == d_top { (0.0, -1.0, circle.radius + min_dist + 0.00001) }
        else { (0.0, 1.0, circle.radius + min_dist + 0.00001) }
    } else {
        let dist = dist_sq.sqrt();
        let inv_dist = 1.0 / dist;
        (dx * inv_dist, dy * inv_dist, circle.radius - dist + 0.00001)
    };
...
#[inline(always)]
fn manage_collision_square_square(entity: &Entity, collision_entity: &Entity, applied_collisions_ptrs_ptr: *mut [f32; 4], thread: usize, entities_size: usize) {
    ...
    let (nx, ny, separation) = if overlap_x < overlap_y {
        (if collision_entity.x - entity.x > 0.0 { 1.0 } else { -1.0 }, 0.0, overlap_x + 0.00001)
    } else {
        (0.0, if collision_entity.y - entity.y > 0.0 { 1.0 } else { -1.0 }, overlap_y + 0.00001)
    };
        ...
        for thread in 0..THREADS {
            ...
            let worker = thread::spawn(move || {
                ...
                for index in chunk[0]..chunk[1] {
                    ...
                    if !entity.replace && entity.mass != f32::MAX {
                        ...
                        if entity.velocity_x > entity.terminal_velocity_in_direction {
                            entity.velocity_x = entity.terminal_velocity_in_direction;
                        } else if entity.velocity_x < -entity.terminal_velocity_in_direction {
                            entity.velocity_x = -entity.terminal_velocity_in_direction;
                        };
                        if entity.velocity_y > entity.terminal_velocity_in_direction {
                            entity.velocity_y = entity.terminal_velocity_in_direction;
                        } else if entity.velocity_y < -entity.terminal_velocity_in_direction {
                            entity.velocity_y = -entity.terminal_velocity_in_direction;
                        };
                        ...
                        } else if entity.x + entity.radius > ROOM_SIZE {
                        ...
                        } else if entity.y + entity.radius > ROOM_SIZE {
                            ...
                            if min_shared_coordinate == grid_index {
                                if entity.body_type + collision_entity.body_type == 2 {
                                    ...
                                    if dsq < (entity.radius + collision_entity.radius) * (entity.radius + collision_entity.radius) {
                                        manage_collision_circle_circle(dsq, entity, collision_entity, applied_collisions, thread, entities_size);
                                    };
                                    ...
                                    if entity.body_type == 0 {
                                        ...
                                        if (cx - collision_entity.x) * (cx - collision_entity.x) + (cy - collision_entity.y) * (cy - collision_entity.y) < collision_entity.radius * collision_entity.radius {
                                            manage_collision_circle_square(cx, cy, collision_entity, entity, applied_collisions, thread, entities_size);
                                        };
                                    } else {
                                        ...
                                        if (cx - entity.x) * (cx - entity.x) + (cy - entity.y) * (cy - entity.y) < entity.radius * entity.radius {
                                            manage_collision_circle_square(cx, cy, entity, collision_entity, applied_collisions, thread, entities_size);
                                        };
                                    };
                                } else {
                                    if f32::abs(collision_entity.x - entity.x) < entity.radius + collision_entity.radius && f32::abs(collision_entity.y - entity.y) < entity.radius + collision_entity.radius {
                                        manage_collision_square_square(entity, collision_entity, applied_collisions, thread, entities_size);
                                    };
                                };
                            };
                        };
